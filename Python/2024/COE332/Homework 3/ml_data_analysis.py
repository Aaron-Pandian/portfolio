...
def compute_statistics(a_list_of_dicts: dict, a_key_string: str):
    """
    Iterates through a list of dictionaries, pulling out values associated \
with a given key. Returns the mean and standard deviation of those values a\
s well as a list of all samples.

    Args:
        a_list_of_dicts (list): A list of dictionaries, each dict should ha\
ve the same set of keys.

        a_key_string (string): A key that appears in each dictionary associ\
ated
        with the desired value (will enforce float type).

    Returns:
        result (list): A list containing the average value and population s\
tandard deviation as floats, and a list of all the masses.
    """
    samples = []
    total_mass = 0.
    for i in a_list_of_dicts:
        try:
            ...
            logging.warning(f'encountered non-float value {i[a_key_string]}\
 in compute_statistics')
        except KeyError:
            logging.warning(f'could not find key {a_key_string} in the {a_l\
ist_of_dicts} dictionary for compute_statistics')
